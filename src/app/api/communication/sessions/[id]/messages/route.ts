...
import { trimConversationContext } from "@/application/communication/prompts";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!allowCommunicationRequest(context.user.id, "message", 60)) return NextResponse.json({ error: "Please slow down for a moment before sending another message." }, { status: 429 });
  ...
  try {
    ...
    const messages = trimConversationContext([...current.messages, userMessage]);
