...
function SportsSummary({ data }: { data?: SportsMarketData }) {
  ...
  return <div className="space-y-5 p-4 sm:p-5"><BestPicks picks={data.bestPicks} />{data.leagues.map((league) => { const matches = data.matches.filter((match) => match.league === league.name); const logo = leagueLogo(league.name); return <section key={league.name} aria-labelledby={`league-${league.name}`}><div className="mb-2 flex items-center justify-between gap-3"><h3 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--foreground-muted)]"><span className="grid size-7 place-items-center rounded-md border border-[var(--border)] bg-[var(--surface-elevated)] text-[10px] font-bold tracking-normal text-[var(--foreground)]" aria-hidden="true">{logo ? <Image src={logo} alt="" width={20} height={20} className="size-5 object-contain" /> : leagueBadge(league.name)}</span><span id={`league-${league.name}`}>{league.name}</span></h3><span className="text-xs text-[var(--foreground-muted)]">{league.matchCount} {league.matchCount === 1 ? "match" : "matches"}</span></div>{matches.length ? <div className="divide-y divide-[var(--border)] rounded-md border border-[var(--border)]">{matches.slice(0, 8).map((match) => <MatchRow key={match.id} match={match} />)}</div> : <p className="rounded-md border border-dashed border-[var(--border)] px-3 py-3 text-xs text-[var(--foreground-muted)]">No scheduled matches in this window.</p>}</section>; })}</div>;
}

function BestPicks({ picks }: { picks: SportsMarketData["bestPicks"] }) { return <section className="rounded-md border border-[var(--border)] bg-[var(--surface-elevated)] p-4" aria-labelledby="best-picks-heading"><div className="flex items-start justify-between gap-3"><div><h3 id="best-picks-heading" className="text-sm font-semibold text-[var(--foreground)]">Best picks for today</h3><p className="mt-1 text-xs leading-5 text-[var(--foreground-muted)]">Highest available market estimates for today&apos;s fixtures.</p></div><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--foreground-muted)]">1X2</span></div>{picks.length ? <div className="mt-3 grid gap-2 sm:grid-cols-3">{picks.map((pick) => <div key={pick.matchId} className="rounded-md border border-[var(--border)] bg-[var(--surface)] p-3"><p className="truncate text-xs font-semibold text-[var(--foreground)]">{pick.selectionLabel}</p><p className="mt-1 truncate text-[11px] text-[var(--foreground-muted)]">{pick.homeTeam} vs {pick.awayTeam}</p><div className="mt-2 flex items-baseline justify-between gap-2"><span className="text-xs text-[var(--foreground-muted)]">{pick.league}</span><strong className="text-sm text-[var(--foreground)]">{pick.probability}%</strong></div></div>)}</div> : <p className="mt-3 rounded-md border border-dashed border-[var(--border)] px-3 py-3 text-xs text-[var(--foreground-muted)]">No verified market picks are available for today&apos;s matches.</p>}<p className="mt-3 text-[10px] text-[var(--foreground-muted)]">Informational market estimates, not guaranteed results.</p></section>; }
